...
boolean WifiManager::connect() {
    ...
    while (retryCount < MAX_RETRY_ATTEMPTS) {
        ...
        Logger::warning("WifiManager::connect()", "Connection attempt failed. Retrying...");
        delay(retryDelay);
        retryCount++;
...
boolean WifiManager::startConfigPortal() {
    ...
    this->server.on("/", HTTP_GET, [this]() { this->renderMainPage(); });
    this->server.on("/setCredentials", HTTP_POST, [this]() {this->getCredentials();});
    this->server.onNotFound([this]() { this->renderMainPage(); });
...
void WifiManager::renderMainPage() {
    String serverLoc = ipToString(this->server.client().localIP());
    Logger::verbose("WifiManager::renderMainPage()", "New connection - IP: " + serverLoc);
    boolean doRedirect = serverLoc != this->server.hostHeader();
    if (doRedirect) {
        this->server.sendHeader(F("Location"), (String)F("http://") + serverLoc, true);
...
boolean WifiManager::storeCredentials(const String& ssid, const String& password) {
    ...
    if (ssid.length() > (PASSWORD_START_ADDR - SSID_START_ADDR - 1)) {
        Logger::verbose("WifiManager::storeCredentials()", "SSID is too long.");
        return false;
    }
